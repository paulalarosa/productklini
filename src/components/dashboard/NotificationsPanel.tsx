                  ...
                  notifications.map((n) => (
                    <div key={n.id} className={`flex items-start gap-2 px-3 py-2 border-b border-border/50 transition-colors ${n.is_read ? "opacity-60" : "bg-accent/20"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {n.message && <p className="text-[10px] text-muted-foreground break-words">{n.message}</p>}
                      </div>
                      <div className="flex items-center gap-0.5 shrink-0">
                        {!n.is_read && (
                          <button onClick={() => markRead(n.id)} className="p-1 rounded hover:bg-accent"><Check className="w-3 h-3 text-muted-foreground" /></button>
                        )}
                        <button onClick={() => deleteNotification(n.id)} className="p-1 rounded hover:bg-destructive/10"><Trash2 className="w-3 h-3 text-muted-foreground" /></button>
                      </div>
                    </div>
                  ))
                )}
